...
CREATE INDEX IF NOT EXISTS idx_events_app_received ON events (app_id, received_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_messages_stuck
    ON messages (locked_until)
    WHERE status = 'in_flight';
CREATE INDEX IF NOT EXISTS idx_messages_endpoint ON messages (endpoint_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_attempts_message ON attempts (message_id, attempt_no);
